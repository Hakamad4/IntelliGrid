...
Para versionamento estamos utilizando Git em combinação com a plataforma Github para orquestrar a colaboração dos esforços do time e para gerenciamento de dependências do projeto utilizamos o Maven. Os membros do grupo estão trabalhando com suas IDEs preferência, seja IntelliJ, Eclipse ou VS Code. Também, abordamos boas práticas , como alguns dos princípios do SOLID, como o Single Reponsability Principle, no qual, tivemos o cuidado de atribuir cada classe uma única responsabilidade.Dessa forma, foi implementado o arquivo `.gitignore` para não permitir o compartilhamento de configurações desses editores ao repositório do projeto.
