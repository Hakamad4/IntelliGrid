...
public class ConsumoController {
	...
	@PostMapping
	public ResponseEntity<ConsumoRequest> cadastrarConsumo(@RequestBody @Valid ConsumoRequest consumoRequest) throws ConsumoNotFoundException {
		consumoService.salvar(consumoRequest);
		return ResponseEntity.status(HttpStatus.CREATED).build();
	}
	
	@GetMapping("/{id}")
	public ResponseEntity<ConsumoResponse> buscarEletroeletronicoPorId(@PathVariable Long id) throws ConsumoNotFoundException {
	...
	@GetMapping
	public ResponseEntity<List<ConsumoResponse>> buscarEletrodomesticos() {
